...
class ZooControllerTest {
    ...
    void shouldUpdateZooSuccessfully() throws Exception {
        // 1. Create an updated zoo: new Zoo(1L, "Updated Zoo Name", "Updated Location",
        // "Updated description", new ArrayList<>(), new ArrayList<>())
        // 2. Set up the mock: when(zooService.updateZoo(eq(1L),
        // any(Zoo.class))).thenReturn(updatedZoo);
        // 3. Use
        ...
        // Your code here:
        Zoo updatedZoo = new Zoo(1L, "Updated Zoo Name", "Updated Location", "Updated description", new ArrayList<>(),
                new ArrayList<>());
    ...
    void shouldReturn404WhenUpdatingNonExistentZoo() throws Exception {
        // 1. Set up the mock to throw an exception: when(zooService.updateZoo(eq(999L),
        // any(Zoo.class))).thenThrow(new IllegalArgumentException("Zoo not found with
        // id: 999"));
        // 2. Use
    ...
    void shouldReturn404WhenDeletingNonExistentZoo() throws Exception {
        // 1. Set up the mock to throw an exception: doThrow(new
        // IllegalArgumentException("Zoo not found with id:
        // 999")).when(zooService).deleteZoo(999L);
    ...
    void shouldHandleMalformedJsonInPutRequest() throws Exception {
        // 1. Use
        // mockMvc.perform(put("/api/zoos/1").contentType(MediaType.APPLICATION_JSON).content("{
        // invalid json }"))
